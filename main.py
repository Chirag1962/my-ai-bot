...
def get_osint_info(phone_number):
    ...
    except: return "❌ OSINT Data Error"
...
def start(message):
    if not check_membership(message.from_user.id):
        ...
        markup.add(types.InlineKeyboardButton("📢 Join Channel", url=CHANNEL_LINK),
                   types.InlineKeyboardButton("🔄 Verify Join", callback_data="check_join"))
...
def ask_for_contact(chat_id):
    ...
    bot.send_message(chat_id, "✅ Welcome! Please share your contact to unlock AI Features.", reply_markup=markup)
...
def verify_join(call):
    ...
    else: bot.answer_callback_query(call.id, "⚠️ Join the channel first!", show_alert=True)
...
def handle_contact(message):
    ...
    bot.send_message(message.chat.id, "✅ **Verified!** Send a photo to remove background.")
...
def process_photo(message):
    ...
    try:
        ...
        if res.status_code == 200:
            bot.send_document(message.chat.id, res.content, visible_file_name='Edited.png', caption="✨ Edited by AI")
...
def ai_chat(message):
    if message.from_user.id in verified_users:
        bot.reply_to(message, "🤖 AI: Send a photo to remove the background!")
